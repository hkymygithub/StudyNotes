...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * 桶的链表还原阈值
     * 当在扩容时（此时HashMap的数据存储位置会重新计算），在重新计算存储位置后，
     * 当红黑树中元素小于等于 6 时，将红黑树转为链表
     */
    ...
    /**
     * 如果类 x 实现了 Comparable 接口则返回 x 的 Class 对象，否则返回 null
     */
    ...
    /**
     * 此 HashMap 已被结构修改的次数 结构修改是指更改 HashMap 中的映射数量
     * 或以其他方式修改其内部结构（例如，重新散列）的那些。
     ...
     * @param evict 如果为 false，则表处于创建模式。此字段将会作为 afterNodeInsertion(evict) 方法参数
    ...
    /**
     * 初始化或者给表扩容(扩为 2 倍).  如果表为空则按照字段保存的容量分配空间
    ...
    final Node<K,V>[] resize() {
        ...
        int newCap, newThr = 0;//  定义新的容量和扩容阈值
        if (oldCap > 0) {//  如果旧容量大于 0，也就是表(Node数组)已经存在
            if (oldCap >= MAXIMUM_CAPACITY) {//  如果旧容量已经是最大值，则无法扩容
                threshold = Integer.MAX_VALUE;//  设置扩容阈值为 int 最大值
                return oldTab;
            }
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY)//  如果旧阈值翻倍后小于最大容量并且旧阈值大于初始容量(16)
                newThr = oldThr << 1; // 阈值翻倍
        }
        else if (oldThr > 0) //  将初始容量设置为 threshold(阈值),当 HashMap 被构造器创建时 threshold 被设置为初始容量
                             //  说明用的是带初始容量参数的构造器
            newCap = oldThr;
        else {               //  阈值为 0 说明初始阈值是默认值，则重新分配值()
                             //  说明用的是无参构造器
            newCap = DEFAULT_INITIAL_CAPACITY;
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
        }
        if (newThr == 0) {//  如果新阈值为 0 则重新计算
            float ft = (float)newCap * loadFactor;
            newThr = (newCap < MAXIMUM_CAPACITY && ft < (float)MAXIMUM_CAPACITY ?
                      (int)ft : Integer.MAX_VALUE);
        }
        threshold = newThr;//  将新阈值赋给 threshold
        @SuppressWarnings({"rawtypes","unchecked"})
        Node<K,V>[] newTab = (Node<K,V>[])new Node[newCap];//  创建对应容量的 Node 数组
        table = newTab;
        if (oldTab != null) {//  如果旧的 table (Node数组)不为空，则需要重新 hash
            for (int j = 0; j < oldCap; ++j) {//  在旧 table 中遍历每个值
                Node<K,V> e;
                if ((e = oldTab[j]) != null) {//  如果 table (Node数组)某处不为 null。说明这里有值需要重新 hash
                    oldTab[j] = null;
                    if (e.next == null)//  如果此处是链表只有这一个元素
                        newTab[e.hash & (newCap - 1)] = e;//  则直接放入在新的 table
                    else if (e instanceof TreeNode)//  如果此处是红黑树则将树拆分并重新放入新的 table 中
                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);
                    else { //  若不止一个元素，并且是以链表形式存在的
                        Node<K,V> loHead = null, loTail = null;
                        Node<K,V> hiHead = null, hiTail = null;
                        Node<K,V> next;
                        do {
                            next = e.next;
                            if ((e.hash & oldCap) == 0) { //  将位置不变的元素连成一条链表
                                if (loTail == null)
                                    loHead = e;
                                else
                                    loTail.next = e;
                        ...
                        if (loTail != null) {//  将链表放进新 table 中
                            loTail.next = null;
                            newTab[j] = loHead;
                        }
                        if (hiTail != null) {//  将链表放进新 table 中
                            hiTail.next = null;
    ...
    /**
     * 将原有链表结构转化为树形结构，但如果 table 太小则只是扩容
     */
    final void treeifyBin(Node<K,V>[] tab, int hash) {
        int n, index; Node<K,V> e;
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)//  如果 table 未初始化并且 table 的长度小于 MIN_TREEIFY_CAPACITY (64) 则只扩容
            resize();
        else if ((e = tab[index = (n - 1) & hash]) != null) {
            TreeNode<K,V> hd = null, tl = null;
            do {
                TreeNode<K,V> p = replacementTreeNode(e, null);//  替换树节点
                if (tl == null)
                    hd = p;
                else {
                    p.prev = tl;
     ...
     *
     * @param m 要被放入此 Map 的 Map 对象
     * @throws NullPointerException 如果参数 Map 等于 null 则抛出异常
     */
     ...
     *
     * @param value 要被测试的 value
     * @return 如果 Map 中有一个或多个特定的 value 则返回 true
     */
    ...
    /**
     * 返回这个 Map 中的 value 集合
     * 这个集合与 Map 相关联，如果 Map 修改了，这个集合中的元素也会相应修改。
     * 除了 Map 自己的迭代器，其他时候对 Map 进行修改，此集合的迭代结果将会不确定
     * 这个 value 集合也支持元素删除，当通过该集合本身迭代器删除元素时其变化会反映到 Map 中，
     * 即在这个集合中删除元素，Map 中对应 key value 也会被删除
     */
    ...
    final class Values extends AbstractCollection<V> {
        ...
        public final void forEach(Consumer<? super V> action) {//  遍历操作 value 集合
            Node<K,V>[] tab;
            if (action == null)
                throw new NullPointerException();
            if (size > 0 && (tab = table) != null) {
    ...
    @Override
    public V getOrDefault(Object key, V defaultValue) {// 如果 Map 中没有对应的值则返回 defaultValue (默认值)
        Node<K,V> e;
        return (e = getNode(hash(key), key)) == null ? defaultValue : e.value;
    }

    @Override
    public V putIfAbsent(K key, V value) {//  如果某个 key 对应的 value 不存在则放入，否则不放入
        return putVal(hash(key), key, value, true, true);
    }

    @Override
    public boolean remove(Object key, Object value) {//  移除元素
        return removeNode(hash(key), key, value, true, true) != null;
    }

    @Override
    public boolean replace(K key, V oldValue, V newValue) {//  如果 key 对应的 value 等于 oldValue 则替换元素
        Node<K,V> e; V v;
    ...
    @Override
    public V replace(K key, V value) {//  替换元素，强制替换
        Node<K,V> e;
        if ((e = getNode(hash(key), key)) != null) {
            V oldValue = e.value;
            e.value = value;
    ...
    /**
     * 返回一个浅拷贝的 HashMap 实例，即 key value 不会被拷贝
     */
    ...
    // 这些方法在序列化 HashSet 的时候也会用到
    final float loadFactor() { return loadFactor; }//  返回负载因子
    final int capacity() {//  返回 Map 容量
